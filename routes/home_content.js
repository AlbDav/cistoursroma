...
router.get('/', function(req, res, next) {
	...
	pool.query('SELECT * FROM categories WHERE category_id = $1', [cat], (err, result) => {
		...
		pool.query('SELECT * FROM products, products_en, prices WHERE category = $1 AND products.product_id = products_en.product_id AND prices.product_id ORDER BY products.product_id', [cat], (err, result) => {
